...
def affineTransform(rawImage, T, interpType):

    # Defining the 2D affine transformation matrix
    transformation_matrix = np.array([[T[0][0], T[0][1], T[0][2]], [T[1][0], T[1][1], T[1][2]]])

    # Getting height and width of the input image
    img_height, img_width = rawImage.shape[:2]
    ...
    # Creating a transformation matrix that translates to ensure positive coordinates
    ...
    transformed_image = cv2.warpAffine(rawImage, final_transform, (canvas_width, canvas_height), flags=interpType)

    # putting transformed image on the canvas
